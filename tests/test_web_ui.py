...
def test_sector_breadth_post_appends_single_stock_output():
    ...
    assert "既存出力" in html
    assert "商社・資源：強い上昇地合い" in html
    assert state.output == "既存出力\n\n■セクター地合\n商社・資源：強い上昇地合い\n"
    assert state.mode == "technical"
    assert state.fundamental_summary_html == ""
    assert state.status == "地合評価を追加しました。 / 評価時点=2026-05-29 09:10"
    assert controller.call["watchlist_entries"] == state.watchlist_with_sectors
