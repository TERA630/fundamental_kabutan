...
def build_technical_summary_html(table: TechnicalSummaryTable) -> str:
    ...
    for rank in RANK_ORDER:
        ...
        sections.append(
            ...
            "<th>25日線乖離</th><th>出来高比</th><th>前日VWAP維持</th>"
            "<th>支持線</th><th>抵抗線</th><th>60日レンジ</th>"
            "</tr></thead><tbody>"
        )
...
def _build_us_market_html(table: UsMarketSummaryTable) -> str:
    ...
    if not rows:
        rows = '<tr><td colspan="6">N/A</td></tr>'
    skipped = ""
    if table.skipped:
        skipped_rows = "".join(
        ...
        skipped = f'<p class="summary-skipped">取得できないUS Market指標:</p><ul class="summary-skipped-list">{skipped_rows}</ul>'
    return (
        f'<h3 class="summary-rank-heading">US Market {escape(table.as_of.strftime("%Y-%m-%d %H:%M"))}</h3>'
        '<div class="table-scroll">'
        '<table class="fundamental-table summary-table us-market-summary-table">'
        "<thead><tr>"
...
def _build_row_html(row: TechnicalSummaryRow) -> str:
    ...
    cells += _build_resistance_lines_cell(row)
    cells += f"<td>{escape(_fmt_position(row.recent60_range_position))}</td>"
    return f"<tr>{cells}</tr>"


def _build_resistance_lines_cell(row: TechnicalSummaryRow) -> str:
    if not row.resistance_lines:
        return "<td>N/A</td>"
    markers = []
    for index, line in enumerate(row.resistance_lines, start=1):
        markers.append(
            '<span class="line-marker resistance-line-marker">'
            f'<span class="line-marker-index">R{index}</span>'
            f'<span>{escape(line.label)} {escape(_fmt_price(line.price))}</span>'
            "</span>"
        )
    return f'<td><span class="line-marker-group">{"".join(markers)}</span></td>'
