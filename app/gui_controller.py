...
from app.domain.models.us_market_summary import SkippedUsMarketSummaryItem, UsMarketSummaryTable
    ...
    def build_technical_summary_table(
        ...
        *,
        ...
        service = TechnicalSummaryService(
            self._build_technical_summary_result,
            build_us_market_summary=self._build_us_market_summary_table,
        )
        return service.build_summary_table(watchlist_entries)

    def _build_us_market_summary_table(self) -> UsMarketSummaryTable:
        try:
            return UsMarketSummaryService().build_summary_table()
        except Exception as exc:
            return UsMarketSummaryTable(
                as_of=datetime.now(ZoneInfo("Asia/Tokyo")),
                rows=(),
                skipped=(SkippedUsMarketSummaryItem(name="US Market", ticker="-", reason=str(exc)),),
            )
