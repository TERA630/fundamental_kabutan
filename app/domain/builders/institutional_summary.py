...
def build_institutional_summary_text(summary: InstitutionalSummary | None) -> str:
    ...
    vwap_suffix = " (日足参考値)" if summary.technical.vwap_is_daily_reference else ""
    return "\n".join(
        [
            "機関投資サマリ",
            ...
            f"機関投資スコア：{summary.score.total}/20点　Fundamental Score：{_fmt_score(summary.fundamental_score)}（{summary.fundamental_rank or 'N/A'}）　Technical：VWAP {summary.technical.vwap}{vwap_suffix} / 5日線 {summary.technical.ma5} / 25日線 {summary.technical.ma25}",
        ]
    )
