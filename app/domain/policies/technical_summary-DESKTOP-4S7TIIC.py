...
def build_primary_support_line(
    *,
    day_low: float | None,
    ma25: float | None,
    recent20_low: float | None,
    ma75: float | None,
    recent60_low: float | None,
) -> TechnicalSummaryLine | None:
    if day_low is None:
        return None
    candidates = tuple(
        line
        for line in (
            TechnicalSummaryLine("25日線", ma25) if ma25 is not None else None,
            TechnicalSummaryLine("20日安値", recent20_low) if recent20_low is not None else None,
            TechnicalSummaryLine("75日線", ma75) if ma75 is not None else None,
            TechnicalSummaryLine("60日安値", recent60_low) if recent60_low is not None else None,
        )
        if line is not None
    )
    if not candidates:
        return None
    return min(candidates, key=lambda line: abs(day_low - line.price))


def build_support_line_comment(
    *,
    support: float | None,
    day_low: float | None,
    latest: float | None,
    ...
    if support is None or atr14 in (None, 0):
        return "N/A"
    lower_threshold = support - 0.25 * atr14
    contact_band = 0.25 * atr14
    check_upper = support + 0.35 * atr14

    if day_low is not None and latest is not None and day_low < lower_threshold and latest > support:
        return "当日支持線割れ戻し"
    if previous_low is not None and previous_close is not None and previous_low < lower_threshold and previous_close > support:
        return "前日支持線割れ戻し"
    if previous_low is not None and latest is not None and previous_low < lower_threshold and latest > support:
        return "当日支持線割れ戻し"
    if day_low is not None and day_low < lower_threshold:
        return "支持線割れ"
    if previous_low is not None and previous_low < lower_threshold:
        return "前日支持線割れ"
    if day_low is not None and abs(day_low - support) <= contact_band:
        return "支持線接触"
    if previous_low is not None and abs(previous_low - support) <= contact_band:
        return "前日支持線接触"
    if day_low is not None and support <= day_low <= check_upper:
        return "支持線確認中"
    if previous_low is not None and support <= previous_low <= check_upper:
        return "前日支持線確認"
    return "N/A"


def build_previous_low_support_comment(
    *,
    previous_low: float | None,
    day_low: float | None,
    latest: float | None,
    ...
    if previous_low is None or atr14 in (None, 0):
        return "N/A"
    lower_threshold = previous_low - 0.25 * atr14
    contact_band = 0.25 * atr14
    check_upper = previous_low + 0.35 * atr14

    if day_low is not None and latest is not None and day_low < lower_threshold and latest > previous_low:
        return "前日安値 割れ戻し"
    if day_low is not None and day_low < lower_threshold:
        return "前日安値 割れ"
    if day_low is not None and abs(day_low - previous_low) <= contact_band:
        return "前日安値 接触"
    if day_low is not None and previous_low <= day_low <= check_upper:
        return "前日安値 確認中"
    return "N/A"
