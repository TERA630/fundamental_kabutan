...
D1 / D2 / D3は単一銘柄詳細画面で詳細分類、状態、主判定を1行の短評にまとめる。
条件と戦略文言は `docs/technical_summary_ranking_spec.md` を正とする。

```text
短評：D1a 戻り途中・25日線接近｜監視優先。D3化なら小さく可
短評：D1b 戻り途中・25日線遠い｜監視優先。深指値は原則不可
短評：D1 判定保留｜判定保留。新規不可
短評：D2 底打ち候補｜支持線反発待ち｜支持線反発候補。原則VWAP回復待ち
短評：D3強｜VWAP維持・出来高伴う｜小さく可。D3内で最有力
短評：D3｜VWAP維持・出来高やや不足｜小さく可。出来高確認
短評：D3弱｜反転形あるも出来高不足｜監視寄り。出来高不足
```

D2 / D3の25日線乖離ラベルを生成できる場合は、主判定の後ろへ補助情報として
`｜25日線奪回接近`、`｜深押し反転` などを併記する。

ランキング一覧上のコードはD1/D2/D3のままとし、D1a/D1bとD3強弱は
単一銘柄詳細画面だけに表示する。
