...
| 当日レンジ | `当日高値-当日安値(値幅 当日値幅率/ATR比)`。当日値幅率は `(当日高値 - 当日安値) / 当日安値` |
...
| 抵抗線 | 抵抗候補のうち現在値より上側で近い2本を `R1:価格線 価格 / R2:価格線 価格` として表示 |
| 60日レンジ | 60日安値から60日高値までの現在値位置 |
...
抵抗線は、欠損値を除外したうえで `価格 > 現在値` の候補を価格の低い順に並べ、現在値の上から近いものを最大2本抽出する。表示では近い順に `R1`、`R2` とする。Web UI では2本の抵抗線であることが分かるように、R1/R2のマーカーと線状の区切りで表示する。
...
|---|---:|---:|---:|---:|---:|---:|---|---|---|---:|
| 信越工業(4063) | 6830円(-0.6%) | -8.8% | 6963-6743(値幅3.3%/0.50ATR) | 6810円(+0.3%) | -8.0%(1.20ATR) | 120% | × | PrevL:6743->20D-L:6682 | R1:PrevH 6963 / R2:20D-H 7100 | 80% |
```
...
| `app/domain/policies/technical_summary.py` | A1/A2/B1/B2/C1/C2/E のランク判定と支持線/抵抗線抽出を純粋関数として実装 |
...
2026-06-10 時点では、Technical Summary のモデル、UseCase、Markdown builder、Web HTML builder、Tkinter の `サマリ出力` 導線、Web UI の `サマリ表示` 導線は実装済みとして扱う。以降は次の順で仕上げる。

### 9.1 第1段階：現行導線の確認と仕様同期

- `docs/current_implementation_spec.md` に `TechnicalSummaryRow` / `TechnicalSummaryTable`、`TechnicalSummaryService`、Web UI の `サマリ表示` を現行仕様として追記する。
- `docs/screen_spec.md` の「Technical タブ選択時のサマリ出力は何もしない」という記述を、Technical Summary Markdown 保存へ更新する。
- `docs/web_ui_design.md` の古い「Technical選択時は何もしない」を、現行の Technical Summary HTML 表示へ更新する。
- `tests/test_technical_summary.py`、`tests/test_gui_controller.py`、`tests/test_web_ui.py` を実行し、現行導線が仕様どおり通ることを確認する。

### 9.2 第2段階：Web UI ダウンロード設計
...
### 9.3 第3段階：US Market セクション追加

- `app/domain/models/us_market_summary.py` を追加し、US Market 行モデルとテーブルモデルを定義する。
- `app/domain/usecases/us_market_summary.py` を追加し、yFinance から対象指標を取得して直近値、前日騰落、5日乖離、25日乖離、RSI14 を計算する。
- `TechnicalSummaryTable` に US Market セクションを持たせるか、Technical Summary の上位DTOを追加する。
- `build_technical_summary_markdown()` と `build_technical_summary_html()` の冒頭に US Market 表を出力する。
- 取得失敗時は Technical Summary 全体を失敗させず、US Market 側だけ `N/A` または skipped 相当で表示する。

### 9.4 第4段階：表示品質とテスト

- `app/static/web.css` で Technical Summary の横スクロール、数値右寄せ、ランク見出し、`N/A` 表示を微調整する。
- Web UI のサマリHTMLがコピー用本文と混ざらないことをテストする。
- Markdown ダウンロード、HTML ダウンロード、未生成時の404または案内メッセージをテストする。
- US Market の計算境界、取得失敗、欠損値、表示順をテストする。
