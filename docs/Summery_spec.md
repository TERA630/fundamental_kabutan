...
Hybrid Summary は Fundamental Summary と Technical Summary を `code4` で合流し、候補抽出用の別ボタンから生成する。既存の Fundamental / Technical サマリとは独立した出力とし、Technical ランクそのものは変更しない。
...
| 出来高 | 当日出来高の20日平均比 |
| 崩れ | 崩れ警戒スコア |
| 抵抗余地 | 最寄り抵抗線までの上値余地。抵抗線がない場合は `Open` |
| 理由 | 分類に使った主要条件 |
