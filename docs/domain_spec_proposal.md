...
冒頭サマリーで表示する `per_level` を、PERとPER判定用業種グループから判定する。
表示文言は `docs/display_spec.md`、判定ロジックは本節を正とする。
...
| 入力 | 内容 |
|------|------|
| `per` | forecast EPS 由来PERを優先し、取得不可の場合のみ market PER を使う |
| `industry` | yFinance の `sector` または `industry`。表示には出さないが、PER判定用業種グループの分類に使う |
...
yFinance の `sector` / `industry` は英語かつ粒度が一定ではないため、Domain Policy 側で `industry` 文字列を正規化して上記グループへ分類する。
分類できない場合は `通常/成長業種` として扱う。

**判定閾値**

| PER判定用業種グループ | `割安PER` | `適正PER` | `高PER` | `超高PER` |
|------------------------|-----------|-----------|---------|-----------|
| `低PER許容業種` | `PER < 8` | `8 <= PER < 15` | `15 <= PER <= 25` | `PER > 25` |
| `通常/成長業種` | `PER < 15` | `15 <= PER < 30` | `30 <= PER <= 50` | `PER > 50` |

PERが `None`、`0` 以下、または数値として不正な場合は分類不能とし、Presenter / Formatter 側で `N/A` 表示する。

### 5.6 ROIC水準分類

冒頭サマリーで表示する `roic_level` を、ROICから判定する。
表示文言は `docs/display_spec.md`、判定ロジックは本節を正とする。
...
ROICが `None`、または数値として不正な場合は分類不能とし、Presenter / Formatter 側で `N/A` 表示する。
