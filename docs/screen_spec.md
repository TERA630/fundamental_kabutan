...
- 株探HTMLフォルダ選択
- 銘柄選択コンボボックス
- `取得` / `最新値手入力` / `地合評価` / `サマリ出力` / `コピー` / `保存` ボタン
- ステータスメッセージ
- 機関投資サマリ固定パネル
...
- `最新値手入力` は Technical タブで選択銘柄の当日現在値、当日高値、当日安値、当日VWAPを入力するウィンドウを開く。`反映して再解析` は評価時点を最新へ戻し、yFinanceキャッシュを書き換えず今回の単一銘柄解析だけに4項目を反映する。
- 手入力値は `当日安値 <= 当日現在値 <= 当日高値`、`当日安値 <= 当日VWAP <= 当日高値` を満たす正の数値とする。yFinanceの当日始値も入力レンジ内でなければならない。
- 手入力で更新しない出来高、5分足RSI、前場・後場VWAP、VWAP維持時間などは取得済みyFinance値を使用する。出力の取得時刻とVWAPには手入力であることを表示する。
...
| 手入力再解析中 | `{name}({code4}) を手入力値で再解析中...` |
| 手入力再解析完了 | `{name}({code4}) を手入力値で再解析しました。 / 反映時刻={YYYY-MM-DD HH:MM}` |
...
Technical 出力は `app/domain/builders/technical_output.py` が組み立てる。表示順は、取得時刻、銘柄ヘッダと先頭サマリ、冒頭短評、崩れ警戒・底打ち初動・ホールド判定、戦略判定、`■モメンタム`、`■当日位置・レンジ`、`■重要価格`、`■前日評価` とする。`地合評価` はボタン押下時だけ、Technical 出力欄の本文末尾へ追記する。
...
短評の後には、将来各条件が成立した場合の行動指針として、前場・後場のVWAP回復戦略を表示する。これは表示時点で売買シグナルが成立していることを意味しない。深押し指値戦略は表示しない。D系は詳細分類ごとに評価記号と文言を切り替える。単一銘柄出力は `TechnicalSummaryRow` ではなく `TechnicalAnalysisResult` から同じ domain policy を使って短評を作る。
