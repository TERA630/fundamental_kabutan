...
```text
短評：{ランク} {状態表示名}｜{6M評価}｜{5日線傾き短評}
```

例:

```text
短評：A1 中期乖離帯｜やや過熱・他指標確認｜5日線良好
短評：C2 崩れ警戒｜監視のみ｜即時崩れ：VWAP明確割れ＋終端位置低下｜5日線下向き
```

短評は Technical ランク、乖離帯に応じた状態表示名、6か月バックテスト評価から生成する。判定と文言は domain policy が担い、`technical_output.py` は判定結果を文字列化する。D1 / D2 / D3 の詳細分類は反転状態の観測材料として残すが、25日線下の6M評価は一律 `25日線奪回待ち` とする。分類条件、判定優先順位、D系の売買行動および補助ラベルは `docs/technical_ranking_spec.md` を正とする。
