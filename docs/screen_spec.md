...
```text
短評：A1弱 上方乖離｜中央値マイナス｜押し目確認｜5日線良好
短評：A1 位置良好｜崩れ警戒｜監視のみ｜即時崩れ：VWAP明確割れ＋終端位置低下｜5日線下向き
```

短評は1Y位置帯と同じ境界の位置ランク、崩れまたは反転の補助状態、1Y位置評価から生成する。判定と文言は domain policy が担い、`technical_output.py` は判定結果を文字列化する。旧D1 / D2 / D3の詳細分類は反転状態と戦略判定の材料として残すが、位置ランクは変更しない。分類条件、判定優先順位、売買行動および補助ラベルは `docs/technical_ranking_spec.md` を正とする。
